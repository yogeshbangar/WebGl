...
class Ball {
    ...
    setScore() {
        ...
        for (var i = 0; i < 6 && i < this.bNo; i++) {
            DrawLbl(mGame.mTex_fonts[i + 2], this.runArray[i] + "", ((i > 2 ? (90 + (this.zoom * (16 / mx))) : 0) - 144 - (this.zoom * (26 / mx))) + (38 + (this.zoom * (7 / mx))) * i, -266 - (this.zoom * ((14 + 3) / mx)), CLR_WHT, 30);
        }
    ...
    update(delta) {
        ...
        // this.bowler.annie.reset(sx)
        // mGame.shadow.position.set(rx * 10, sy, sz);
            ...
            if (this.anim > 0) {
                ...
                if (this.anim > 80) {
                    //mGame.mTex_fonts[0].visible = false;
                      ...
                      DrawLbl(mGame.mTex_fonts[1], 'GET A 1000 AMAZON VOUCHER. ', 0, -80, CLR_WHT, 18);
...
class Game {
	...
	init() {
		...
		var texture_bowler = loadTexture(BOWLER_64);
		...
		this.planStamp[0] = TexSingle(texture_stump, 2, 1, 2, 40);
		...
		this.planStamp[0].mBody.position.set(0, 17, 265);
		this.planStamp[1].mBody.position.set(0, 15, -8);
		var scl = .7;
		...
		var bowler = TexSingle(texture_bowler, 16, 16, 256, 40);
		...
		// this.myAudiohappy = document.getElementById("myAudiohappy");
		// this.myAudiosad = document.getElementById("myAudiosad");
		// this.myAudiobg = document.getElementById("myAudiobg");
	...
	animate() {
		...
		switch (GameScreen) {
			case GAMEMENU:
      var scl = .6;
      console.log("Check~~~~00~Menu "+this.counter);
      this.rect.visible = true;
      ...
      DrawTexture(this.presents, 0, -170, 91 * scl, 18 * scl);
        ...
        // this.mSel%=2;
			...
			case GAMEPLAY:
				...
				// }
				if (this.mBall.start == 1) {
					DrawLbl(this.mTex_fonts[0], 'GOOD LUCK', 0, -140, CLR_WHT, 30);
				...
				if (this.mBall.start == 2) {
					DrawLbl(this.mTex_fonts[0], 'TAP TO START', 0, -170, CLR_WHT, 30);
					this.mTex_fonts[1].visible = false;
	...
	setScreen(scr) {
		...
		switch (GameScreen) {
			...
			this.rect.visible = true;
				...
				DrawLbl(this.mTex_fonts[0], 'TAP TO HIT', 0, -180 + 125, CLR_WHT, 30);
				DrawLbl(this.mTex_fonts[1], 'THE SHOTS', 0, -140 + 125, CLR_WHT, 30);
	...
	drawResult() {
		...
		scl = 1;
		DrawTexture(this.result, 0, -30, 363 * scl, 512 * scl);
		var scl = .6;
		...
		DrawLbl(this.mTex_fonts[7], 'RUN AND    OFFERS', 10, oy - 70, CLR_BLUE, 28);
		DrawLbl(this.mTex_fonts[8], '' + runs, -110 - (runs > 9 ? 10 : 0), oy - 70, CLR_RED, 28);
		DrawLbl(this.mTex_fonts[9], '' + ofr, 20, oy - 70, CLR_RED, 28);
		scl = .42;// + rx * .1;
