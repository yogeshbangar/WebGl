...
class Runner {
    ...
    update() {
        ...
        if(mGame.go > 3)
        {
            if(this.status == 1){
                ...
                if(this.roll.annie.currentTile > 10){
                    this.roll.annie.tileDisplayDuration = 25;
        ...
        if (this.gComp == 0) {
            ...
            if (this.y < -5) {
              if(mGame.myAudioFall.paused)
                mGame.myAudioFall.play();
            ...
            if (this.spd > 0) {
                ...
                if(this.gComp == 1){
                    this.gComp++;
                    var bi = (mGame.lJump) % mGame.mBuilding.length;
                ...
                mGame.mTargetPlan.visible = false;
                mGame.skyTarget.visible = true;
                var bi = (mGame.lJump) % mGame.mBuilding.length;
                var y = mGame.mBuilding[bi].position.y + mGame.mBuilding[bi].scale.y * .5 + .61;
                if (this.gComp < 36) {
                    mGame.skyTarget.position.set(0, .1 * (36 - this.gComp) + y, mGame.mTargetPlan.position.z + .3);
                } else {
                    mGame.skyTarget.position.set(0, y, mGame.mTargetPlan.position.z + .3);
                    DrawLbl(mGame.mTex_fonts[0], "Isko laga dala toh\nfull family Jingalala", 0,-450, CLR_BLUE, 44);
                }
                this.x *= .9;
                ...
                if (this.gComp > 20){
                    // DrawLbl(mGame.mTex_fonts[0], "Isko laga dala toh\nfull family Jingalala", 0, 450, CLR_WHT, 44);
    ...
    tap(dir) {
        // if (this.vy == 0 || this.vx == 0)
        {
            ...
            if (dir == UP && this.onBase == 0) {
                ...
                var val = mGame.mBuilding[mGame.lJump].scale.z + mGame.mBuilding[mGame.lJump].position.z ;
                // console.log(val,this.z,mGame.mBuilding[mGame.lJump].scale.z,mGame.mBuilding[mGame.lJump].position.z);
                if(mGame.mBuilding[mGame.lJump].scale.y>mGame.mBuilding[noe].scale.y){
                    this.roll.annie.tileDisplayDuration = 60;
                    this.status = 2;
                    if(mGame.mBuilding[mGame.lJump].position.z < -10){
                        this.status = 1;
                    }
                }else
                    this.status = 1;
