...
class Game {
    ...
    shaderAdd() {
        ...
        this.renderTarget = new THREE.WebGLRenderTarget(window.innerWidth, window.innerHeight, { minFilter: THREE.LinearFilter, magFilter: THREE.NearestFilter, format: THREE.RGBFormat });
    ...
    init() {
        ...
        this.cylinders = [];
        var geometry = new THREE.CylinderGeometry(8, 8, 40, 32, 1, true, Math.PI * .4, Math.PI * 2);
        var material = new THREE.MeshLambertMaterial({ map: new THREE.TextureLoader().load('assets/lava/lavatile.jpg'), side: THREE.DoubleSide, color: 0xff0000 });
        for (let i = 0; i < 0; i++) {
        ...
        loader.load('assets/power.obj', function (obj) {
        ...
        loader.load('assets/WarpRide.gltf', function (obj) {
    ...
    animate() {
        ...
        switch (GameScreen) {
            case GAMEPLAY:
                ...
                break;
            case GAMEOVER:
                DrawTexture(this.reload, 0, 360, 128 * (this.mSel == 1 ? 1.1 : 1), 128 * (this.mSel == 1 ? 1.1 : 1));
        ...
        if (this.gameCounter < 300) {
            ...
            for (let i = 0; i < this.boosters.length; i++) {
                ...
                if (this.spd < 1) {
                    ...
                    if (this.boosters[i].visible && CircRectsOverlap(this.MovingCube.position.x, this.MovingCube.position.z, .1, 2, this.boosters[i].position.x, this.boosters[i].position.z, .1)) {
                        this.boosters[i].visible = false;
                        this.boosCount++;
                    }
                }
            ...
            for (let i = 0; i < this.boosters.length; i++) {
                if (this.boosters[i].position.z > 20) {
                    ...
                    this.boosters[i].position.x = random(-10, 10) * .1;
    ...
    setScreen(scr) {
        ...
        switch (GameScreen) {
            ...
            case GAMEPLAY:
                ...
                for (let i = 0; i < this.boosters.length; i++) {
                    this.boosters[i].position.set(random(-10, 10) * .1, 0, -50 - i * 20);
                }
                this.MovingCube.position.set(0, 0, -5);
            ...
            case GAMEOVER:
                ...
                DrawLbl(this.mTex_fonts[0], 'Catch: ' + this.boosCount, 0, -160, CLR_WHT, 100);
                DrawLbl(this.mTex_fonts[1], 'Total: ' + this.boosTotal, 0, 0, CLR_WHT, 100);
                DrawLbl(this.mTex_fonts[2], 'Score: ' + (this.boosCount * 10 - this.boosTotal), 0, 160, CLR_WHT, 100);
