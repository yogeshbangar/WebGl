class Ball {
    constructor(_ball, _batsman, _bowler, _filder, _shadow) {
        ...
        this.shadow = _shadow;
        this.camerax = 0;
        this.shadow.rotation.set(-Math.PI * .50, 0,0);
        this.spd = new THREE.Vector3( 0, 0, 0 );
        this.gravty = .01;
        this.bowler.mBody.scale.set(.5, 2.0, 1);
    ...
    update(delta) {
        ...
        if(this.anim && this.filder.annie.currentTile<124){
            ...
            if(this.filder.annie.currentTile>20 && mGame.camera.position.x==0 ){
                if(this.filder.mBody.scale.x < 0)
                    new TWEEN.Tween(mGame.camera).to({ position: {x: 30}}, 300).start();
                else
                    new TWEEN.Tween(mGame.camera).to({ position: {x: -30}}, 300).start();
            }
        }
        if(this.ball.position.z > 270 && this.ball.position.z < 278 && this.status == 0 && this.filder.annie.currentTile>25 && this.filder.annie.currentTile<50){
            if(this.filder.mBody.scale.x > 0 && this.ball.position.x < 0)
                this.ball.visible = false;
            if(this.filder.mBody.scale.x < 0 && this.ball.position.x > 0)
                this.ball.visible = false;
            this.status =1;
            console.log('~~~1~~~~~~',this.ball.position,this.filder.annie.currentTile,this.status);
        }
        if(this.filder.annie.currentTile>=100 && this.status == 1 && this.anim){
            console.log('~~~2~~~~~~',this.ball.position,this.filder.annie.currentTile,this.status);
            if(this.ball.visible == false){
                if(this.filder.mBody.scale.x < 0)
                    this.ball.position.set(28,22,200);
                else
                    this.ball.position.set(-28,22,200);
        ...
        if(this.ball.position.x < -56){
            ...
            switch(this.short){
                case 0://Plat
                    this.spd.set(.14,0.8,1.6);//Left
                    this.gravty = .011;
                break;
                case 1://Plat
                    this.spd.set(.52,0.82,1.6);//Left
                    this.gravty = .011;
                break;
                case 2://Top
                    this.spd.set(.14,1.1,1.6);//left
                    this.gravty = .014;
                break;
                case 3://Top
                    this.spd.set(.53,1.1,1.6);//right
                    this.gravty = .014;
                break;
                case 4://ground
                    this.spd.set(.14,0.1,1.6);//right
                    this.gravty = .054;
                break;
                case 5://ground
                    this.spd.set(.52,0.0,1.6);//right
                    this.gravty = .054;
                break;

            }
        }
        this.shadow.position.set(this.ball.position.x, .21, this.ball.position.z);//0-30
        this.shadow.visible = this.ball.visible ;
        this.ball.rotation.set(mGame.counter*.1, mGame.counter*.1,mGame.counter*.1);
        if(this.ball.position.z > 450 || (this.spd.y < 0 && this.ball.position.y< 50&& this.status==2)){
            this.ballset();
            console.log('~~~3~~~~~~',this.ball.position,this.filder.annie.currentTile,this.status);
            if(this.bNo >= 6){
