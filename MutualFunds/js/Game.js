...
class Game {
    ...
    init() {
        ...
        this.camera.rotation.set(-Math.PI*.04, Math.PI*ry, Math.PI*rz);//30
        ...
        texture_ground.wrapS = THREE.RepeatWrapping; texture_ground.wrapT = THREE.RepeatWrapping; texture_ground.repeat.set(16, 16);
        this.Ground = new THREE.Mesh(new THREE.PlaneBufferGeometry(580,580, 1, 1), new THREE.MeshBasicMaterial({ map: texture_ground }));
        ...
        this.sPublic = new THREE.Mesh(new THREE.CylinderGeometry(500, 300, 200, 16, .5, true, Math.PI * .5, Math.PI * 1), material);
        ...
        this.pitch = new THREE.Mesh(new THREE.PlaneBufferGeometry(70*.5, 280*.5, 1, 1), material);
        this.scene.add(this.pitch);
        this.pitch.rotation.set(-Math.PI * .5, 0, Math.PI*.5);
        this.pitch.position.set(0, .1, 0);



        var material = new THREE.MeshBasicMaterial({ side: THREE.DoubleSide, map: texture_citi });
        texture_citi.wrapS = THREE.RepeatWrapping; texture_citi.wrapT = THREE.RepeatWrapping; texture_citi.repeat.set(8, 1);
        this.citi = new THREE.Mesh(new THREE.CylinderGeometry(300, 300, 16, 16, .5, true, Math.PI * .5, Math.PI * 1), material);
        ...
        var shadow = new THREE.Mesh(new THREE.CircleGeometry(1, 8), new THREE.PointsMaterial({ color: 0x000000, opacity: 0.8, transparent: true }));
        this.scene.add(shadow);

        var ball = new THREE.Mesh(new THREE.SphereGeometry(1, 16, 16), new THREE.MeshLambertMaterial({ map: texture_balltex }));
        this.scene.add(ball);
    ...
    animate() {
        ...
        switch (GameScreen) {
            ...
            case GAMEHELP:
                ...
                DrawLbl(this.mTex_fonts[0], "SWIPE LEFT\nTO MOVE LEFT", -234, 560, CLR_WHT, 30);
                DrawLbl(this.mTex_fonts[1], "SWIPE RIGHT\nTO MOVE RIGHT",  234, 560, CLR_WHT, 30);
                break;
            case GAMEPLAY:
                this.rect.visible = false;
                this.mBall.update(this.delta * 1000);
                DrawLbl(this.mTex_fonts[0], ""+mGame.mBall.run, 170, -475, CLR_WHT, 70);
                break;
            case GAMERESULT:
                DrawTexture(this.mTex_over, 0, 0, 720, 1280);
                DrawLbl(this.mTex_fonts[0], ""+this.mBall.run, -90+sx, -12, CLR_WHT, 80);
                break;
    ...
    setScreen(scr) {
        ...
        this.mTex_over.visible = this.mTex_help.visible = this.mTex_home.visible = this.mTex_jersey.visible = this.mTex_mf.visible = false;
